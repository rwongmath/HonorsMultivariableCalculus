...
Now let us consider linear maps $\R^2 \to \R^2$.
...
\fixthis{explicit, proof, picture}
...
That is, the coordinates\SubIndex{coordinates} of a vector in an orthonormal basis $\{\bm{b_i}\}$ of $\R^n$ are exactly given by the dot products $\bm{v}\cdot \bm{b_1}$.
...
\begin{example}
    ...
    $$\{\bm{v} = \langle x, y, z \rangle \in \R^2 \ | \ ax+by+cz = 0\}$$
    
    Let us take for example $\bm{n} = \langle 3, 2, 1\rangle \in \R^3$ in blue.
    
\end{example}

Similarly, we can use vector addition to describe arbitrary planes in $\R^3$ - that is, in order to define a plane $P$ in $\R^3$ (not necessarily passing through the origin), we simply need to describe a point on the plane $P$, and a normal vector $\bm{n} \in \R^3$.
