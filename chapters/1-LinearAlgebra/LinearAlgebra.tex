...
\begin{proposition}
    Given a non-zero vector $\bm{v} \in \R^n$, its \textbf{direction vector}\define{direction vector} $\bm{e_v}$ is unique. 
\end{proposition}

The general strategy to prove that something is unique is as follows:

\fixthis{Explain more}
...
\end{definition}
 ...
 \begin{center}        
    ...
    There are several reasons that bases are useful!  
...
See the exercises (\ref{problem:scalarpreserving}) for examples of maps that preserve one property, but not the other.
...
In later sections we will see how to generalize the proposition above (theorem \ref{thm:matrixmultislinear}).
...
\begin{definition}
    Let $f: V \to W$ be a linear map.  We say that $f$ is \textbf{injective}\define{injective} or \textbf{one-to-one}\define{one-to-one} (or sometimes, $f$ is an \textbf{injection}) if the following holds:
...
\begin{definition}
    Let $f: V \to W$ be a linear map.  We say that $f$ is \textbf{surjective}\define{surjective} or \textbf{onto}\define{onto}  (or sometimes, $f$ is a \textbf{surjection}) if the following holds:
...
\begin{definition}\label{bijective}
    Let $f: V \to W$ be a linear map.  We say that $f$ is \textbf{bijective}\define{bijective}  (or sometimes, $f$ is a \textbf{bijection})f $f$ is both injective and surjective.
...
\begin{theorem}\label{thm:matrixmultislinear}
...
\begin{definition}\label{transpose}
    Given a matrix $A = [a_{i,j}] \in M_{n\times n}(\R)$, the \textbf{transpose matrix}\define{transpose matrix} is the matrix $A^\intercal$ defined by $$A^\intercal = [b_{i,j}] \in M_{n\times n}(\R)$$
    where $b_{i,j} := a_{j,i}$.
\end{definition}
...
\fixthis{left inverse, right inverse}
...
\end{proposition}
    ...
    A linear transformation $T: V \to W$ is an \textbf{isomorphism of vector spaces}\define{isomorphism} if $T$ satisfies any of the following equivalent conditions:
    ...
    If $T : V \to W$ is an isomorphism, we say that $V$ and $W$ are \textbf{isomorphic vector spaces}\define{isomorphic}.
\end{definition}
...
From our discussion about dimension earlier, the dimension\SubIndex{dimension}\footnote{That is, the cardinality of any basis of $V$.  See definition \ref{def:dimension}} of a vector space $V$ is an invariant of $V$).  For example, the dimension of $\R^n$ is $n$\footnote{see example \ref{dimrn}}.
...
\begin{theorem}[Expansion along the $i$-th row]\label{expansiondet}
...
This generalizes to $n \times n$ matrices and their associated linear transformations $T:\R^n\to \R^n$.  Let us write $\bm{v_i} = T(\bm{e_i})$.
