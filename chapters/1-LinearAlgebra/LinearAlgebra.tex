...
Furthermore, it also makes sense to combine the points perspective with the vector perspective:  It makes sense to add the add the \textit{displacement} from Los Angeles to Austin to the \textit{location} New York City.  
...
We can interpret this geometrically through the following calculation:
...
Parallel vectors are so named because they give rise to parallel lines \fixthis{ref later}
...
\begin{problem}{parallelvectors}
    Are these vectors parallel?  Are they equivalent?
\end{problem}

\begin{problem}{addsubtractvectors}
...
When faced with an abstract definition like this, \fixthis{it's especially important to first find concrete examples that manifest}.  Luckily, from our discussion in the previous section, we have in fact checked most of these axioms:
